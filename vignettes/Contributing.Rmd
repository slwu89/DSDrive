...
Thank you for taking the time to contribute to DSDrive (d. suzkuii gene drive simulation).
...
We periodically merge `dev` into `master` for small release updates. These releases will appear on the [GitHub releases page](https://github.com/slwu89/DSDrive/releases). 
...
runs during pull requests to `[main, dev]` and uses our [codecov.io integration](https://app.codecov.io/gh/slwu89/DSDrive)
...
*tests/* - tests for **DSDrive**

## Pull Requests

Pull requests from collaborators should never be submitted directly to `main`, always to `dev`.
Periodically we will create a versioned release and merge `dev` with `main`.
